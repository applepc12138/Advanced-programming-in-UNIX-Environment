/*************************************************************************
	...
	> Created Time: 2020年10月06日 星期二 20时41分47秒
 ************************************************************************/
#include <stdio.h>
#include <stdlib.h>
#include <pthread.h>
...
struct threadpool_t *threadpool_create(int min_thr_num, int max_thr_num, int queue_max_size)
{
    ...
    do{
        ...
        {
            pthread_create(&(pool->threads[i]), NULL, threadpool_thread, (void *)pool);//工作线程
            printf("start thread 0x%x...\n", (unsigned int)pool->threads[i]);
        }
        pthread_create(&(pool->adjust_tid), NULL, adjust_thread, (void *)pool);//管理线程
        return pool;
...
int threadpool_all_threadnum(struct threadpool_t *pool)
{
    int all_threadnum = -1;
    pthread_mutex_lock(&(pool->lock));
...
int threadpool_busy_threadnum(struct threadpool_t *pool)
{
...
int main(void)
{
    struct threadpool_t *thp = threadpool_create(3,100,100);    /*线程池里最小3个线程，最大100个，队列最大值100*/
    printf("pool inited");
