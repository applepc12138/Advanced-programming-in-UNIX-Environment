...
struct myevent_s
{
    ...
    void *arg;          //指向自己结构体指针
    void (*call_back)(int fd,int events,void *arg); //回调函数,但epoll_wait函数返回后被调用
...
/*
 * 构造(封装)一个自定义事件。包括fd，这个fd的回调函数，还有一个额外的参数项
...
void eventadd(int efd, int events, struct myevent_s *ev)
{
    ...
    int op = 0;
    epv.data.ptr = ev; // ptr指向一个结构体（之前的epoll模型红黑树上挂载的是文件描述符confd和lfd，现在是ptr指针）
...
//listenfd就绪后的回调函数
/*  当有文件描述符就绪, epoll_wait返回, 调用该函数与客户端建立链接 */
void acceptconn(int lfd,int events,void *arg)
{
    ...
    do {
        ...
        int flag = 0;
        if((flag = fcntl(confd, F_SETFL, O_NONBLOCK)) < 0) {//将confd也设置为非阻塞
            printf("%s: fcntl nonblocking failed, %s\n", __func__, strerror(errno));
            break;
        }
        eventset(&g_events[i], confd, recvdata, &g_events[i]); //找到合适的节点之后，将其添加到监听树中，并监听读事件
...
void recvdata(int fd, int events, void *arg)
{
    ...
    if (len > 0) {
		...
		//这里服务器若直接调用write函数向客户端发数据可能会导致服务器阻塞
